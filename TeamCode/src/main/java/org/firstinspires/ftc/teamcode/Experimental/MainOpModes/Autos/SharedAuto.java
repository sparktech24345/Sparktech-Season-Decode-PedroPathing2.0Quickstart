...
import static org.firstinspires.ftc.teamcode.Experimental.MainOpModes.Teleops.MainTeleOpBlue.angleOffset;
...
@Config
@Autonomous(name = "\uD83D\uDD35 Shared Auto BLUE", group = "BBB") // 🔵
public class SharedAuto extends OpMode {
    ...
    private boolean isShootingAtSharedAuto = false;
...
//        if(wentTooNumber2 >0) cameraCase = 1; not needed with new rolers
    ...
    public void firingTurret(boolean shouldFire) {
        ...
        double distanceToWallOdometry = calculateDistanceToWallInMeters(robot.getCurrentPose(), targetXHere, targetYHere);
        double rotationToWallOdometry = - calculateHeadingAdjustment(robot.getCurrentPose(), Math.toDegrees(robot.getCurrentPose().getHeading()), targetXHere, targetYHere);
...
//                    new StateAction("IntakeMotor","FULL_REVERSE"),
    ...
    public void convertPoses() {
        ...
        bezierHelper1Shared = convertPose(bezierHelper1Shared);
        small_triangle_shootShared = convertPose(small_triangle_shootShared);
        intermediaryEvadePipe = convertPose(intermediaryEvadePipe);
    }
